  ...
  btn.addEventListener("click", async (event) => {
    ...
    if (event.target.type === "submit") {
      ...
      if (selected.length === 0) {
        ...
        axios
          .post(`http://localhost:4500/home?arrItem=${selected}`)
          .then((data) => {
  ...
  btnApprove.addEventListener("click", async (event) => {
    ...
    if (event.target.type === "button") {
      ...
      if (selected.length === 0) {
        ...
        axios
          .post(
            `http://localhost:4500/manager-home?actionType=Approve&arrItem=${selected}`
          )
          .then((data) => {
            console.log("data:", data);
  ...
  btnReject.addEventListener("click", async (event) => {
    ...
    if (event.target.type === "button") {
      ...
      if (selected.length === 0) {
        ...
        axios
          .post(
            `http://localhost:4500/manager-home?actionType=Reject&arrItem=${selected}`
          )
          .then((data) => {
            console.log("data:", data);
            window.location = "/manager-home";
