...
const {
  registerEmployee,
  loginUser,
} = require("../services/empAuthService.js");
...
async function processSignupSubmission(req, res, next) {
  ...
  if (token === "EMP_EXISTS") {
    ...
    message =
      "The Employee Id or Email address entered is not valid. Please try again.";
  } else if (token) {
    message = "Thank you for registration,Please login to proceed further ";
    res.render("login", { layout: "main2", message });
  } else {
    message = "An error has occurred. Please try again.";
  }

  res.render("signup", { layout: "main2", message });
}
