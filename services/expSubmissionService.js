...
async function getManagerList(empId, status) {
  return await empExpense
    .find({ managerId: empId, expStatus: status })
    .lean()
    .exec();
}
...
async function updateExpenseRecord(fields, employeeId) {
  ...
  return await empExpense
    .updateOne(
      { _id: recordId, empId: employeeId },
      {
        $set: {
          expType: fields.expType,
          expDate: fields.expDate,
          expReceipt: fields.expReceipt,
          expAmount: fields.expAmount,
          expStatus: "Saved",
        },
      }
    )
...
async function submitApprovalExpense(idArray, mngrId) {
  return await empExpense
    .updateMany(
      { _id: { $in: idArray } },
      { $set: { expStatus: "Pending", managerId: mngrId } }
    )
    .exec();
}
...
async function approvedExpenses(idArray) {
  return await empExpense
    .updateMany({ _id: { $in: idArray } }, { $set: { expStatus: "Approved" } })
    .exec();
}

async function rejectedExpenses(idArray) {
  return await empExpense
    .updateMany({ _id: { $in: idArray } }, { $set: { expStatus: "Rejected" } })
    .exec();
}
